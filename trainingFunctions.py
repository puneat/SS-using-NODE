...
#     norm(dim), nn.ReLU(inplace=True), nn.AdaptiveAvgPool1d(1), 
    ...
    fc_layers = [norm(dim), nn.ReLU(inplace=True), nn.AdaptiveAvgPool1d(1), Flatten(), nn.Linear(dim,16), nn.ReLU(inplace=True), nn.Dropout(p=0.2, inplace=True), nn.Linear(16,2)]
