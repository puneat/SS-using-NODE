...
def loadData(path, modulation, type='signal', snr=True):
	...
	if snr and type=='signal':
		data.columns = column_headers
		print('\n Loaded ' + str(modulation) + ' ' + type + ' data with shape: '
			+ str(data.shape))

	elif snr and type=='noise':
		data.columns = column_headers[:513]
		print('\n Loaded ' + type + ' data with shape: ' + str(data.shape))
	elif not snr:
		data.columns = column_headers
		print('\n Loaded '+str(modulation) + ' ' + type + ' data with shape: '
			+ str(data.shape))
